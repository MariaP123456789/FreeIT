...
logging.basicConfig(filename='log_task10.log', filemode='w', format='%(asctime)s - %(message)s', level=logging.INFO)
...
def execute_raw_sql_query_with_exeption(query_text):
	try:							
...
execute_raw_sql_query_with_exeption(query_text_create_tables)

# второй запрос - заполнение
dt_currencies = get_currencies()
...
for i in range(task8_addition.days_number):	
	...
	dt = get_courses(strftime)['rates']
	for key, value in dt.items():
		new_value = float(value)
		query_text_insertion += f" ('{key}','{new_value}', '{strftime}'), "
	today = today - timedelta(days=1)

execute_raw_sql_query_with_exeption(query_text_insertion[:-2])

# третий запрос - выборка
data = execute_raw_sql_query_with_exeption("SELECT value FROM public.currency_data \
